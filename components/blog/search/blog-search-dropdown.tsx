...
import { Search, MapPin, FileText, Info, Calendar, X } from "lucide-react"
import { type SearchSuggestion, typeLabels, typeColors, groupSuggestionsByType } from "./blog-search-suggestions"
...
export function BlogSearchDropdown({
  isOpen,
  suggestions,
  onSuggestionClick,
  isMobile = false,
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex-shrink-0 px-4 py-4 border-b border-gray-200 bg-gradient-to-r from-emerald-50 to-teal-50">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => onSuggestionClick({ id: "close", title: "", type: "city", icon: null, query: "" })}
                    ...
                    {typeSuggestions.map((suggestion) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <h5 className="text-base font-medium leading-tight text-gray-900 mb-1">{suggestion.title}</h5>
          ...
          <div className="flex-shrink-0 px-4 py-3 bg-gray-50 border-t border-gray-200">
            <div className="flex items-center justify-center text-sm text-gray-500">
              <span className="flex items-center gap-2">
                ...
                {Object.keys(groupedSuggestions).length} категорий • {suggestions.length} разделов
              </span>
            </div>
          </div>
        </div>
  ...
  return (
    <div
      className={`absolute top-[calc(100%+6px)] left-0 right-0 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden transition-all duration-300 ease-out transform ${
                ...
                {typeSuggestions.map((suggestion) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {suggestion.count && <span className="text-xs text-gray-500 mt-1 block">{suggestion.count}</span>}
                    </div>
                  </button>
                ))}
              </div>
